...
export function wheelDeltaToPixels(
  ...
  const modeFactor =
    payload.deltaMode === WheelEvent.DOM_DELTA_LINE
      ? RAIL_LINE_DELTA_PX
      : payload.deltaMode === WheelEvent.DOM_DELTA_PAGE
        ? rail.clientWidth
        : 1
...
export function measureSlideStride(rail: HTMLDivElement): number {
  ...
  if (rail.clientWidth > 0) return rail.clientWidth
  return 0
}
...
export function measureLoopSegmentWidth(
  ...
  if (firstReal && firstTailClone) {
    ...
    if (segmentWidth > 0) return segmentWidth
  }

  if (stride > 0) return stride * total
  return rail.clientWidth
}

export function choosePrimaryAxisDelta(
  deltaX: number,
  deltaY: number,
): number {
  const horizontalWeight = Math.abs(deltaX)
  const verticalWeight = Math.abs(deltaY)
  if (horizontalWeight === 0 && verticalWeight === 0) return 0
