import offlinePlaceholder from '@/assets/offline-placeholder.svg'
...
export async function fetchPicsumImages(
  ...
  const apiBase = import.meta.env.VITE_PICSUM_API_BASE?.trim()
  if (!apiBase) {
    throw new Error('Missing VITE_PICSUM_API_BASE')
  }
