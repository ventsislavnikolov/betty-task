...
import { useWheelRailMotion } from '@/hooks/useWheelRailMotion'
...
export function InfiniteCarousel({ items }: InfiniteCarouselProps) {
  ...
  const onScroll = useCallback(() => {
    const track = containerRef.current
    if (!track || isLoopAdjustmentRef.current) return
  ...
  return (
    ...
    >
      <div className="relative" data-testid="carousel-shell">
        <div
          className={[
            'carousel-track relative box-border flex gap-[var(--carousel-gap)] overflow-x-auto px-2 py-6 focus:outline-none [--carousel-gap:0.75rem] sm:[--carousel-gap:1rem] sm:px-3 sm:py-8',
          ...
          {renderedSlides.map((slide, renderIndex) => {
            ...
            return (
              <article
                ...
                className={[
                  'w-[var(--carousel-card-width)] flex-none aspect-[16/9] overflow-hidden rounded-xl bg-black/20',
                  ...
                  expandedRenderIndex === renderIndex
                    ? 'z-30 scale-[1.35] -translate-y-1 shadow-[0_32px_56px_-28px_rgba(0,0,0,0.9)]'
                    : 'z-0 scale-100',
      ...
      <p
        aria-hidden="true"
        className="mt-3 text-center font-body text-[10px] uppercase tracking-[0.3em] text-muted/50"
      >
