...
describe('fetchPicsumImages', () => {
  ...
  test('throws when VITE_PICSUM_API_BASE is missing', async () => {
    vi.stubEnv('VITE_PICSUM_API_BASE', '')

    await expect(fetchPicsumImages(3)).rejects.toThrow(
      'Missing VITE_PICSUM_API_BASE',
    )
  })

  test('maps API response to CarouselItem with aspectRatio and srcSet', async () => {
    vi.stubEnv('VITE_PICSUM_API_BASE', 'https://picsum.photos')
  ...
  test('builds deterministic offline placeholder items', () => {
    ...
    expect(items[0].src).toMatch(
      /^(data:image\/svg\+xml|\/src\/assets\/offline-placeholder\.svg)/,
    )
